jQuery(document).ready(function () {
    ...
    "=": {
      ...
      search: function (value, comparison) {
        if (typeof comparison == "object" && comparison.length > 0) {
          return (
            value != "" &&
            comparison.filter((text) => value.search(text) > -1).length > 0
          );
        }
  ...
  function getSearchState() {
    var local_storge_table = localStorage.getItem(
      "DataTables_list-customer_/customer/"
    );
  ...
  $(".checkbox-element").on("click", function () {
    if (
      $(".checkbox-element:checked").length == $(".checkbox-element").length
    ) {
  ...
  $(".em-importer .btn.btn-fillter").click(function (e) {
    ...
    if (popup.find(".dtsb-criteria").length === 1) {
      setTimeout(function () {
        $(".dtsb-clearAll.dtsb-button,.custom-btn.revert").css(
          "display",
          "none"
        );
      }, 10);
    ...
    if (
      popup.is(":visible") &&
      popup.find(".dtsb-titleRow .title").length === 0
    ) {
  ...
  $(document).on("click", ".daterangepicker .ranges ul li:first", function (
    e,
    picker
  ) {
    e.preventDefault();
  ...
  $(".em-importer .btn-time").daterangepicker({
    ...
    ranges: {
      ...
      "1 tháng qua": [
        adjustDateForOverflow(
          moment().subtract(1, "month").date(today).startOf("day")
        ), // Adjust to valid start of 1 month ago
        moment().date(yestoday).endOf("day"), // End of today's date
      ],
      "3 tháng qua": [
        adjustDateForOverflow(
          moment().subtract(3, "month").date(today).startOf("day")
        ),
        moment().date(yestoday).endOf("day"),
      ],
      "6 tháng qua": [
        adjustDateForOverflow(
          moment().subtract(6, "month").date(today).startOf("day")
        ),
        moment().date(yestoday).endOf("day"),
      ],
      "1 năm qua": [
        adjustDateForOverflow(
          moment().subtract(12, "month").date(today).startOf("day")
        ),
        moment().date(yestoday).endOf("day"),
      ],
    },
  ...
  $(".em-importer .btn-time").on("apply.daterangepicker", function (
    ev,
    picker
  ) {
  ...
  $(document).on("click", $checkboxes, function () {
    var countCheckedCheckboxes = $(
      '.table-list-customer td input[type="checkbox"]:checked'
    ).length;
    if (countCheckedCheckboxes > 0) {
      $(".em-importer li.status").show();
      $(
        ".em-importer .count-checked, .modal-copy-phone .form-group .total"
  ...
  $(".copyAllphone").click(function () {
    ...
    if (numbers.trim() === "") {
      $("#modal-warning-edit").addClass("is-active");
      $("body").addClass("overflow");
      $(".modal-warning .modal-body p span.txt_append").text(
  ...
  jQuery(
    'ul.tabNavigation li[rel="settings"],ul.tabNavigation li[rel="history"]'
  ).click(function () {
