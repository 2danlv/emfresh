<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            $selected_locations = get_post_meta($post_id, 'location', false); 
        ...
        foreach ($locations as $index => $loc) {
            $key = json_encode($loc); 
            $existing_locations_map[$key] = $loc; 
        }
        ...
        if (isset($_POST['locations']) && is_array($_POST['locations'])) {
            foreach ($_POST['locations'] as $index => $location) {
                ...
                //var_dump($existing_locations_map);

                if (!empty($location_data['district']) && !empty($location_data['ward'])) {
                    // delete_post_meta($post_id, 'location');
                    if (array_key_exists($new_key, $existing_locations_map)) {
                        //delete_post_meta($post_id, 'location');
                        var_dump(array_key_exists($new_key, $existing_locations_map));
                        var_dump($existing_locations_map);
                                            ...
                                            <?php foreach ($selected_locations as $index => $location): ?>
                                                <div class="address-group">
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-sm-9">
                                                            <select id="district_<?php echo $index; ?>" name="locations[<?php echo $index; ?>][district]" class="district-select form-control" required >
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-sm-9">
                                                            <select id="ward_<?php echo $index; ?>" name="locations[<?php echo $index; ?>][ward]" class="ward-select form-control" required >
